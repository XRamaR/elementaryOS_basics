...
sudo apt install ffmpeg -y && sudo add-apt-repository ppa:obsproject/obs-studio
sudo apt update
...
#microsoft fonts
sudo apt install ttf-mscorefonts-installer -y && mkdir .fonts && cd .fonts && wget -qO- http://plasmasturm.org/code/vistafonts-installer/vistafonts-installer | bash
...
#atom
sudo apt install software-properties-common apt-transport-https wget && wget -q https://packagecloud.io/AtomEditor/atom/gpgkey -O- | sudo apt-key add -
sudo add-apt-repository "deb [arch=amd64] https://packagecloud.io/AtomEditor/atom/any/ any main" && sudo apt install atom -y
